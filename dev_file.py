...
if __name__ == "__main__":
    ...
    test_data, test_nodes = net_a.load_data(control_of)
    rvals, p = net_a.corr_matrix(test_data, corr_type='Spearman', z_trans=False)
    threshold_matrix = net_a.significance_check(p, rvals, 1, names=test_nodes, include_Negs=True)
    per = net_a.percentile(threshold_matrix, 0.2)
    G = net_a.networx(per, test_nodes)

    test_data1, test_nodes1 = net_a.load_data(ChR2_of)
    rvals1, p1 = net_a.corr_matrix(test_data1, corr_type='Spearman', z_trans=False)
    threshold_matrix1 = net_a.significance_check(p1, rvals1, 1, names=test_nodes1, include_Negs=True)
    ...
    chr2_hub, top_chr2 = net_a.find_my_hubs(net_a.grab_node_attributes(G1, compress_to_df=True))
    control_hub, top_control = net_a.find_my_hubs(net_a.grab_node_attributes(G, compress_to_df=True))
